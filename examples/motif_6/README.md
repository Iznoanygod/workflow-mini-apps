# Adaptive Training Motif Workflow Mini-app
Workflow mini-app built based on an Adaptive Training workflow.

The workflow treats training as a standalone workflow, in the style of a hyperparameter
optimization campaign. HPC simulations generate the training data, then an ensemble of
models with sampled hyperparameter configurations is trained concurrently. After each
training round, an analysis task evaluates the accuracy of the models and the quality of
the training data: low-accuracy models are terminated and respawned with new
configurations, and when data coverage is insufficient, new data-generating simulations
are spawned before the next round. The workflow ends when all models converge or the
maximum number of rounds is reached. The ensemble size, number of rounds, hyperparameter
search space, and thresholds are all configured in `config.yaml`.
...
- `$ cd workflow-mini-apps/examples/motif_6`
- `$ dragon -s miniapp_adaptive.py`
