# Multistage Pipeline Motif Workflow Mini-app
Workflow mini-app built based on a Multistage Pipeline workflow.

The workflow executes a sequence of HPC simulation stages with data dependencies between them,
in the style of a high-throughput virtual screening pipeline.
After each stage, an AI analysis task consumes the stage output and filters the candidate
ensemble: only candidates whose score passes the stage's `filter_threshold` advance to the
next (higher fidelity, smaller ensemble) stage. The AI analysis controls the branching of
the pipeline — if no candidates pass a filter, the remaining stages are not triggered.
The stages, ensemble sizes, fidelities, and thresholds are all configured in `config.yaml`.
...
- `$ cd workflow-mini-apps/examples/motif_2`
- `$ dragon -s miniapp_pipeline.py`
